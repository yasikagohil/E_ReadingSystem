        ...
        if(request.getParameter("username")!=null)
        {
            ...
            if(result.getStatus()== CredentialValidationResult.Status.VALID)
            {
                ...
                if(request.getRequestURI().contains("Login.xhtml"))
                {
                    ...
                    {
                       request.getServletContext().getRequestDispatcher("/index.xhtml?faces-redirect=true").forward(request, response);
                       return status;
                    }
                    else if(result.getCallerGroups().contains("User"))
                    {
                       request.getServletContext().getRequestDispatcher("/user_homepage.xhtml?faces-redirect=true").forward(request, response);
                       return status;
                    }
                }
            }
